...
async def get_last_messages():
    ...
    This process is repeated 3 times with a 10-second delay between repetitions.
    """
    ...
    async with Client("my_account") as app:
        # Outer loop to repeat the process 3 times
        for iteration in range(999999):
                ...
                async for message in app.get_chat_history(CHANNEL_USERNAME, limit=3):
                    ...
                    if processed_url:
                        ...
                        if processed_url not in sent_urls:
                            message_to_send = f"New CA: \n- {processed_url}\n"
                            try:
                                await app.send_message(
                                    chat_id=TARGET_USER,
                                    text=message_to_send
                                )
                                print(f"Successfully sent NEW processed URL part to {TARGET_USER}")
                                sent_urls.add(processed_url) # Add to set of sent URLs
                            except Exception as e:
                                print(f"Failed to send processed URL part to {TARGET_USER}: {e}")
                        else:
            ...
            # Wait for 10 seconds before the next iteration, unless it's the last one
            if iteration < 999999: # Only wait after the first two iterations
